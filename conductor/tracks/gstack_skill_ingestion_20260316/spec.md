...
We will leverage uv to fetch, select, rename, and transform skills from `gstack` for deployment via Chezmoi to Gemini, Claude, Cursor, and Codex agents.
This fulfills the "uv-powered upstream skill assembly" goal outlined in our current design documentation.

# Functional requirements
- **Upstream ingestion**: Configure `http_archive` in `MODULE.uv` to fetch the `gstack` repository at a pinned commit.
- **Skill selection and renaming**: Implement or extend uv rules (e.g., in `tools/agents/`) to:
...
- **Chezmoi integration**:
    - Update `src/chezmoi/.chezmoiexternals/upstream-skills.toml.tmpl` to use the uv-produced archives via `file://` URLs.
    - Ensure the template can dynamically resolve uv output paths.
...
- **Reproducibility**: Maintain complete reproducibility by pinning upstream sources and using uv for all transformations.
- **Validation**: Integrate the new pipeline with our existing uv validation and drift testing infrastructure.

# Acceptance criteria
- `gstack` is successfully configured as an upstream source.
- uv targets produce skill archives that correctly filter and rename `gstack` skills.
