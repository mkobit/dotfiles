...
def _zsh_config_impl(ctx):
    ...
    # Add header with runtime date expansion
    header = ctx.attr.header
    if header:
        # Create header with proper date expansion - use double quotes for command substitution
        header_lines = header.split("\\n")
        for i, line in enumerate(header_lines):
            if "$(date)" in line:
                # Use double quotes to allow command substitution
                expanded_line = line.replace("$(date)", "$(date '+%Y-%m-%d %H:%M:%S')")
                if i == 0:
                    cmd.append('echo "{}" > {}'.format(expanded_line, output.path))
                else:
                    cmd.append('echo "{}" >> {}'.format(expanded_line, output.path))
            else:
                # Use single quotes for literal strings
                if i == 0:
                    cmd.append("echo '{}' > {}".format(line, output.path))
                else:
                    cmd.append("echo '{}' >> {}".format(line, output.path))
    else:
