"""Rules for generating Rectangle configuration files."""

# Define the RectangleConfigInfo provider
RectangleConfigInfo = provider(
    doc = "Information about Rectangle configuration",
    fields = {
        "settings": "Dictionary of Rectangle settings",
        "includes": "List of other Rectangle configs to include",
        "platform_specific": "Platform-specific settings",
        "variant_settings": "Variant-specific settings (work, personal, etc.)",
    },
)

def _rectangle_config_impl(ctx):
    """Implementation of rectangle_config rule."""
    ...
    for include in ctx.attr.includes:
        include_info = include[RectangleConfigInfo]
...
rectangle_config = rule(
    implementation = _rectangle_config_impl,
    attrs = {
        "settings": attr.string_dict(
            doc = "Dictionary of Rectangle settings",
            default = {},
        ),
        "includes": attr.label_list(
            doc = "Other rectangle_config targets to include",
            default = [],
            providers = [RectangleConfigInfo],
        ),
...
def _get_plist_type(value):
    """Determine the plist type based on value."""
    if value.isdigit():
        return "integer"
    else:
        return "string"

def _rectangle_config_generator_impl(ctx):
    """Implementation for rectangle_config_generator rule that generates configuration files."""

    # Get the RectangleConfigInfo provider from the config target
    config_info = ctx.attr.config[RectangleConfigInfo]
    ...
    output = ctx.actions.declare_file(ctx.attr.name + ".plist")
    content = []
    ...
    content.append('<dict>')
    content.append('<!-- Generated Rectangle config file - DO NOT EDIT -->')
    ...
    for key, value in config_info.settings.items():
        content.append('    <key>{}</key>'.format(key))
        type_string = _get_plist_type(value)
        content.append('    <{}>{}</{}>\n'.format(type_string, value, type_string))
...
rectangle_config_generator = rule(
    implementation = _rectangle_config_generator_impl,
    attrs = {
        "config": attr.label(
            doc = "The rectangle_config target to use",
            mandatory = True,
            providers = [RectangleConfigInfo],
        ),
        "variant": attr.string(
