"""Rules for generating iTerm2 configuration files."""

# Define the iTerm2ConfigInfo provider
iTerm2ConfigInfo = provider(
    doc = "Information about iTerm2 configuration",
    fields = {
        "settings": "Dictionary of iTerm2 settings",
        "includes": "List of other iTerm2 configs to include",
        "platform_specific": "Platform-specific settings",
        "variant_settings": "Variant-specific settings (work, personal, etc.)",
    },
)

def _iterm2_config_impl(ctx):
    """Implementation of iterm2_config rule."""
    ...
    for include in ctx.attr.includes:
        include_info = include[iTerm2ConfigInfo]
...
iterm2_config = rule(
    implementation = _iterm2_config_impl,
    attrs = {
        "settings": attr.string_dict(
            doc = "Dictionary of iTerm2 settings",
            default = {},
        ),
        "includes": attr.label_list(
            doc = "Other iterm2_config targets to include",
            default = [],
            providers = [iTerm2ConfigInfo],
        ),
...
def _iterm2_config_generator_impl(ctx):
    """Implementation for iterm2_config_generator rule that generates configuration files."""

    # Get the iTerm2ConfigInfo provider from the config target
    config_info = ctx.attr.config[iTerm2ConfigInfo]
    ...
    content.append("<dict>")
    content.append("<!-- Generated iTerm2 config file - DO NOT EDIT -->")
    ...
    for key, value in config_info.settings.items():
        ...
        content.append("    <{}>{}</{}>\n".format(
            "string" if isinstance(value, str) else "real" if isinstance(value, float) else "integer" if isinstance(value, int) else "true" if value == "true" else "false",
            value,
            "string" if isinstance(value, str) else "real" if isinstance(value, float) else "integer" if isinstance(value, int) else "true" if value == "true" else "false"
        ))
...
iterm2_config_generator = rule(
    implementation = _iterm2_config_generator_impl,
    attrs = {
        "config": attr.label(
            doc = "The iterm2_config target to use",
            mandatory = True,
            providers = [iTerm2ConfigInfo],
        ),
        "variant": attr.string(
