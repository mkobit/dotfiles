"""Rules for generating ripgrep configuration files."""

# Define the RipgrepConfigInfo provider
RipgrepConfigInfo = provider(
    doc = "Information about ripgrep configuration",
    fields = {
        "settings": "Dictionary of ripgrep settings",
        "includes": "List of other ripgrep configs to include",
        "platform_specific": "Platform-specific settings",
        "variant_settings": "Variant-specific settings (work, personal, etc.)",
    },
)

def _ripgrep_config_impl(ctx):
    """Implementation of ripgrep_config rule."""
    ...
    for include in ctx.attr.includes:
        include_info = include[RipgrepConfigInfo]
...
ripgrep_config = rule(
    implementation = _ripgrep_config_impl,
    attrs = {
        "settings": attr.string_dict(
            doc = "Dictionary of ripgrep settings",
            default = {},
        ),
        "includes": attr.label_list(
            doc = "Other ripgrep_config targets to include",
            default = [],
            providers = [RipgrepConfigInfo],
        ),
...
def _ripgrep_config_generator_impl(ctx):
    """Implementation for ripgrep_config_generator rule that generates configuration files."""

    # Get the RipgrepConfigInfo provider from the config target
    config_info = ctx.attr.config[RipgrepConfigInfo]
    ...
    output = ctx.actions.declare_file(ctx.attr.name + ".rc")
    content = []

    # Generate file header
    content.append("# Generated ripgrep config file - DO NOT EDIT")
...
ripgrep_config_generator = rule(
    implementation = _ripgrep_config_generator_impl,
    attrs = {
        "config": attr.label(
            doc = "The ripgrep_config target to use",
            mandatory = True,
            providers = [RipgrepConfigInfo],
        ),
        "variant": attr.string(
