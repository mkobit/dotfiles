"""Rules for generating bat configuration files."""

# Define the BatConfigInfo provider
BatConfigInfo = provider(
    doc = "Information about bat configuration",
    fields = {
        "settings": "Dictionary of bat settings",
        "includes": "List of other bat configs to include",
        "platform_specific": "Platform-specific settings",
        "variant_settings": "Variant-specific settings (work, personal, etc.)",
    },
)

def _bat_config_impl(ctx):
    """Implementation of bat_config rule."""
    ...
    for include in ctx.attr.includes:
        include_info = include[BatConfigInfo]
...
bat_config = rule(
    implementation = _bat_config_impl,
    attrs = {
        "settings": attr.string_dict(
            doc = "Dictionary of bat settings",
            default = {},
        ),
        "includes": attr.label_list(
            doc = "Other bat_config targets to include",
            default = [],
            providers = [BatConfigInfo],
        ),
...
def _bat_config_generator_impl(ctx):
    """Implementation for bat_config_generator rule that generates configuration files."""

    # Get the BatConfigInfo provider from the config target
    config_info = ctx.attr.config[BatConfigInfo]
    ...
    output = ctx.actions.declare_file(ctx.attr.name + ".conf")
    content = []

    # Generate file header
    content.append("# Generated bat config file - DO NOT EDIT")
...
bat_config_generator = rule(
    implementation = _bat_config_generator_impl,
    attrs = {
        "config": attr.label(
            doc = "The bat_config target to use",
            mandatory = True,
            providers = [BatConfigInfo],
        ),
        "variant": attr.string(
