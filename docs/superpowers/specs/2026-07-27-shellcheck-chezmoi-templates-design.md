...
- Single-machine rendering can produce spurious findings, not just miss coverage: a data-dependent loop (e.g. a `range` over configured extensions) that happens to iterate zero times on the rendering machine can trigger a real shellcheck finding (e.g. `SC2034` unused variable) for a variable that *is* used once the loop actually iterates elsewhere.
  ...
  Policy: a scoped inline `# shellcheck disable=<code>` directive at the specific line, with a comment stating it's a render artifact and why (for example `# shellcheck disable=SC2034 # unused when .gemini.extensions is empty on this machine; consumed inside the range loop below` — a second `#`, not `--`, separates the directive from the reason; shellcheck's directive parser only accepts `key=value` pairs after `# shellcheck` and errors on `--` with `SC1072`/`SC1073`, confirmed empirically during implementation).
