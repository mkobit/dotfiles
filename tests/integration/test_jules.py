...
def test_jules_cli_available(host):
    ...
    assert result.rc == 0, f"jules command not found in zsh environment.\nstderr: {result.stderr}"
    assert "jules" in result.stdout, "command -v jules did not print a path containing 'jules'."
...
def test_jules_cli_help(host):
    ...
    assert "Usage: jules" in result.stdout or "Usage:" in result.stdout, "Did not find expected help output."
