...
def _validate_plan_version(plan: Mapping[str, object]) -> None:
    version = plan.get("plan_version")
    if type(version) is not int or version != _PLAN_VERSION:
        raise AcquisitionError(f"unsupported plan version: {version!r}")
...
def build_plan(externals: Mapping[str, object]) -> dict[str, object]:
    ...
    for name, raw_definition in sorted(externals.items()):
        ...
        for skill, state in sorted(skills.items()):
            ...
            selected.add(skill)
            select = (
                _relative(skill_file, f"external skill {skill!r}.select")
                if skill_file is not None
                else skill
                if skills_root == "."
                else posixpath.join(skills_root, skill)
            )
...
def _fetch(url: str, destination: Path) -> None:
    ...
    if url.startswith("file://"):
        ...
        shutil.copyfile(source, destination, follow_symlinks=False)
        return
    # The scheme is explicitly restricted to HTTP(S) before using urlopen.
    open_url = urllib.request.__dict__["urlopen"]
...
def render_synthetic_externals(
    ...
    *,
    ...
    for raw_source in sources:
        ...
        expected_root = source.get("expected_root", ".")
        if not isinstance(expected_root, str):
            raise AcquisitionError(f"source {source_id!r}.expected_root must be a path")
        _relative(expected_root, f"source {source_id!r}.expected_root")
        args.extend(("--expected-root", expected_root))
        lines.extend(
            (
