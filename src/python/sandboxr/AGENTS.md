...
- **Autonomous.**
`sandboxr run --profile autonomous -- TOOL ARGS`.
...
- `uv run pytest src/python/sandboxr` — unit coverage for config, backend dispatch, and bwrap argv.
- `sandboxr doctor --profile autonomous` — probes inside the sandbox confirm credentials are hidden, root is read-only, project is writable, and commit signing is disabled.
- `sandboxr doctor --profile readonly` — same probes plus project read-only assertion.
