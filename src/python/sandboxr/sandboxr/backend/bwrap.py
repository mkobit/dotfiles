...
# Claude Code has a working fallback (open+truncate+write in place instead
# of atomic rename), so this doesn't break functionality, but it is a real
# correctness downgrade -- the write is no longer atomic for a credentials/
# session file that's shared with every other concurrent claude session on
# the host. Documented here so the tradeoff is explicit.
RW_HOME_PATHS = (
    ".claude",
    ".claude.json",
    ".gemini",
...
def _build_mounts(spec: SandboxSpec, mask_paths: Sequence[str]) -> list[str]:
    ...
    # Ad-hoc path binds from --ro / --rw flags.
