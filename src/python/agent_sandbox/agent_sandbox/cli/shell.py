...
def shell(
    ...
    tty: Annotated[bool, typer.Option("--tty/--no-tty", "-t", help="Allocate a pseudo-TTY.")] = True,
    show_command: Annotated[bool, typer.Option("--show-command", help="Print bwrap invocation instead of running.")] = False,
    project_write: Annotated[bool | None, typer.Option("--project-write/--no-project-write")] = None,
    ...
    Examples:
      agent-run shell                           # sandboxed bash, default profile
      agent-run shell --profile untrusted       # untrusted profile
      agent-run shell --ssh-agent               # with host SSH agent forwarded
      agent-run shell --ro ~/.npmrc             # expose ~/.npmrc read-only
    """
    _refuse_if_nested()
