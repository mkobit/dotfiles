...
- **Secrets** (`[data.secrets]` in `.chezmoi.toml.tmpl`, prompted once via `promptStringOnce`, skipped under `env "CI"`): rendered into `dot_config/zsh/private_secrets.zsh.tmpl` — a normal chezmoi-owned target (`private_` → `0600`), not merged into `.zshrc` directly. The `zsh.config_fragments` entry (`.chezmoidata/zsh.toml` → `.chezmoitemplates/shell/secrets.sh`) is a static one-line `source` of that file, so the fragment's content — and therefore its hash in `.zshrc` — never changes when a secret's value does. Never written to a git-tracked file. Blank answer = the rendered file is empty, harmless no-op. Add more secrets to `private_secrets.zsh.tmpl` rather than inventing a per-tool prompt/deploy mechanism, and never inline a secret's value directly into a fragment template (fragments land in `.zshrc`, which is `644` and gets diffed in plaintext) — e.g. a project's `.mcp.json` can reference `${STITCH_API_KEY}` directly with no chezmoi involvement beyond this export.
