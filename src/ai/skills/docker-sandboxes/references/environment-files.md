...
Resolve toolchain bumps (e.g. Bun or Node via mise) in-place inside an existing sandbox using `sbx exec <name> -- mise install -y`, or tear down the environment with `sbx env rm` for a clean kit rebuild.
