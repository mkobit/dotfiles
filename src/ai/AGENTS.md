...
The catalog is `src/chezmoi/.chezmoidata/ai/skills.toml`: upstream sources pin a commit `ref` + archive `sha256`; skill entries are `"present"` or `"absent"` (never delete a key — set it `"absent"` so `.chezmoiremove.tmpl` prunes the installed copy).
